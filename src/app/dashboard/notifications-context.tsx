...
export function NotificationsProvider({ children }: { children: React.ReactNode }) {
    ...
    const hasInitializedNativeBridgeRef = React.useRef(false);
        ...
        if (!isAllNotificationsLoading && allNotifications && currentUserProfile) {
            ...
            if (typeof window !== 'undefined' && (window as any).ReactNativeWebView) {
                ...
                if (!hasInitializedNativeBridgeRef.current) {
                    ...
                    hasInitializedNativeBridgeRef.current = true;
                    return;
                }

                const newUnreadNotification = notificationsForUser.find(notif => !notif.read && !seenNotificationIdsRef.current.has(notif.id));
                seenNotificationIdsRef.current = unreadIds;

                if (newUnreadNotification) {
                    (window as any).ReactNativeWebView.postMessage(JSON.stringify({
                        type: 'lista-show-local-notification',
                        title: newUnreadNotification.title,
                        body: newUnreadNotification.description,
                        data: {
                            notificationId: newUnreadNotification.id,
                            notificationType: newUnreadNotification.type,
                        },
                    }));
                }
            }
