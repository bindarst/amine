...
export default function DiaperOrderForm({ ward, orderState, onOrderChange }: DiaperOrderFormProps) {
  ...
  const handleApplyNeed = (diaperId: string, quantity: number) => {
    internalDispatch({ type: 'SET_MODE', diaperId, mode: 'pieces' });
    internalDispatch({ type: 'SET_QUANTITY', diaperId, quantity });
  };
        ...
        {sortedAndFilteredDiapers.map((diaper) => {
          ...
          return (
            <Card
              key={diaper.id}
              className={cn(
                "flex flex-col shadow-sm border",
                activeSwipeTarget === diaper.id && "ring-2 ring-primary",
                isStockExceeded ? "border-destructive bg-destructive/10" : "border-border"
              )}
            ...
            >
              <CardHeader
                className="p-4 flex-row items-center justify-between space-x-3 border-b bg-background"
              >
                <div className="flex items-center gap-2.5 flex-1 min-w-0">
                  <span
                    className="h-3 w-3 rounded-full border border-border shrink-0"
                    style={{ backgroundColor: diaper.hexColor || 'hsl(var(--muted))' }}
                    aria-hidden="true"
                  />
                  <CardTitle className="truncate text-base font-semibold text-foreground">{diaper.name}</CardTitle>
                </div>

                {shouldShowParLevelSection && (
                  <button
                    type="button"
                    onClick={() => handleApplyNeed(diaper.id, parLevel ?? 0)}
                    className="inline-flex shrink-0 items-center gap-2 rounded-md border border-border bg-muted px-3 py-1.5 text-sm font-medium text-foreground transition-colors hover:bg-muted/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                    title="Remplir la commande avec le besoin"
                  >
                    <Target className="h-3.5 w-3.5 text-muted-foreground" />
                    <span className="text-xs text-muted-foreground">Besoin</span>
                    <span className="font-bold">{parLevel}</span>
                  </button>
                )}
              </CardHeader>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground w-full justify-center text-center">
                    <div className="flex items-center justify-center gap-1">
                      ...
                      <span className="font-black text-lg text-violet-600 dark:text-violet-400">
                        {stockQuantity.toLocaleString('fr-FR')}
                      </span>
                    </div>
                    <div className="flex items-center justify-center gap-1">
                      <span className="text-muted-foreground">Restant: </span>
                      <span className="font-black text-xl text-fuchsia-600 dark:text-fuchsia-400">
                        {remainingStock.toLocaleString('fr-FR')}
                      </span>
                    </div>
                  </div>
