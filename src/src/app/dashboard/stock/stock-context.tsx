...
interface StockContextType {
  ...
  directStockDistribution: (items: DeliveryItem[], reason: string) => Promise<void>;
}
...
export function StockProvider({ children }: { children: React.ReactNode }) {
  ...
  const checkLowStockAndNotify = React.useCallback((diaper: WithId<Diaper>, oldQuantity: number, newQuantity: number) => {
    if (newQuantity < diaper.lowStockThreshold && oldQuantity >= diaper.lowStockThreshold) {
      const notification = {
        type: 'stock',
        title: 'Stock Bas',
        description: `Le stock de "${diaper.name}" est bas (${newQuantity} pièces restantes).`,
        data: {
          diaperId: diaper.id,
  ...
  const manualStockUpdate = React.useCallback(async (diaperId: string, newQuantity: number) => {
    ...
    try {
        ...
        const description = `${userProfile?.displayName || user.email} a changé le stock de "${diaperInfo?.name || 'un article'}" de ${oldQuantity} à ${newQuantity}.`;
        // In-app Notification
        const notification = {
            type: 'info' as const,
  ...
  const directStockDistribution = React.useCallback(async (items: DeliveryItem[], reason: string) => {
    if (!firestore || !user) throw new Error("Firestore not initialized");
    ...
    try {
        ...
        const description = `${userProfile?.displayName || user.email} a sorti ${totalItems} article(s) du stock. Raison : ${reason}`;
        ...
        // Email Notification
        const subject = `[Lista] Distribution Directe de Stock`;
